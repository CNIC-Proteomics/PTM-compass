...
def decnumber(decnum,d):
    decnum = int(str(decnum).replace("f","").replace(".",""))
    d = round(float(d),int(decnum))
...
def applyGM (row1,dfUserFile,Error,Theo_mh_column_name,DM_PDM_column_name,userlist_unique,group_name,decnum):
    cond = 0 
    e = "YES"
    final_group_name = " "
    try: 
    ...
    if e != "NO":
        ...
        if row1[group_name]==" ": 
            row1[group_name]= str(decnumber(decnum,row1[DM_PDM_column_name]))
        return row1
...
def main(configurationfile,PDMTable,UserFile,):
    ...
    decnum= config["GroupMaker_Parameters"].getint("decnum") # Decimals points required if deltamass is annotated as a group
    ...
    dfPDM = dfPDM.apply(lambda y:applyGM(y,dfUserFile,float(Error),Theo_mh_column_name,DM_PDM_column_name,userlist_unique,group_name,decnum), axis = 1)
