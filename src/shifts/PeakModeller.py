...
def main(args):
    ...
    # # Get the new output file names
    # filename_with_extension = os.path.basename(h_outfile)
    # # Extract the part up to the last underscore
    # filename_base = filename_with_extension[:filename_with_extension.rfind('_') + 1]
        ...
        for j in range(3):
            ...
            if j==0:
                ...
                if separate_modelling==True:
                    ...
                    h_outfile = os.path.join(os.path.dirname(args.infile),'DMHistogram_target.tsv')
                    t_outfile = os.path.join(os.path.dirname(args.infile),'DMTable_target.feather')
                else:
                    continue
                ...
                if separate_modelling==True:
                    ...
                    df_copy=df.loc[bools].reset_index(drop=True)
                    h_outfile = os.path.join(os.path.dirname(args.infile),'DMHistogram_decoy.tsv')
                    t_outfile = os.path.join(os.path.dirname(args.infile),'DMTable_decoy.feather')
                else:
                    continue

            else:
                h_outfile = os.path.join(os.path.dirname(args.infile),'DMHistogram.tsv')
                t_outfile = os.path.join(os.path.dirname(args.infile),'DMTable.feather')
                df_copy=df
                # pass
